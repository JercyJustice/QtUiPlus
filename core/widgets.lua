...
--     back its parts in `qtpParts` so the caller can show and hide each region
...
local function Part(control, region)
  ...
  if not control.qtpParts then control.qtpParts = {} end
  table.insert(control.qtpParts, region)
  return region
end
...
-- Checkbox
--
-- One shared mark for both QtUiPlus controls and adapted native CheckButtons.
...
function U.SetCheckboxIndicator(button, checked, markInset)
  ...
  local mask = button.qtpCheckboxMask
  if not mask then
    ...
    button.qtpCheckboxMask = mask
  end
  U.SetColor(mask, M.Unpack(M.color.background))
  mask:Show()

  local mark = button.qtpCheckboxMark
  if not mark then
...
-- the state. Clicking the swatch opens QtUiPlus's own RGB dialog.
--
...
--
-- The saturation/value square and hue strip are QtUiPlus's own, drawn as plain
...
local function SetPartsShown(control, show)
  ...
  if control.qtpParts then
    local i
    for i = 1, table.getn(control.qtpParts) do
      local region = control.qtpParts[i]
      if region then
...
local function AttachDragMarker(area, marker, ticker, onMove)
  ...
  -- QtUiPlus uses it, so each read keeps its own ok/value pair.
...
local function EnsureColorDialog()
  ...
  local preview = CreateFrame("Frame", "QtUiPlusColorPickerPreview", dialog)
  ...
  local square = CreateFrame("Frame", "QtUiPlusColorPickerSquare", dialog)
  ...
  local marker = CreateFrame("Button", "QtUiPlusColorPickerMarker", square)
  marker:SetWidth(10)
  marker:SetHeight(10)
  ...
  local strip = CreateFrame("Frame", "QtUiPlusColorPickerHue", dialog)
  ...
  local hueThumb = CreateFrame("Button", "QtUiPlusColorPickerHueThumb", strip)
  hueThumb:SetWidth(8)
  ...
  for i = 1, table.getn(CHANNELS) do
    ...
    local slider = U.CreateSlider(dialog, {
      name = "QtUiPlusColorPicker" .. string.upper(key),
      text = CHANNELS[i][2],
      width = SLIDER_WIDTH,
      min = 0,
      max = 255,
  ...
  local cancel = U.CreateButton(dialog, {
    name = "QtUiPlusColorPickerCancel",
    text = "Cancel",
    width = 100,
    height = 22,
...
function U.HideConfirm(owner)
  ...
  if owner and confirmDialog.qtpOwner ~= owner then return end
  confirmDialog:Hide()
end
