...
-- every other core file assumes QtUiPlus already exists when it loads.
...
function U.Print(msg)
  Output("|cff33c0ffQt|cffffffffUiPlus|r: " .. tostring(msg))
end

function U.Error(msg)
  Output("|cff33c0ffQt|cffffffffUiPlus|r |cffff5555error|r: " .. tostring(msg))
end
...
function U.Debug(msg)
  ...
  Output("|cff33c0ffQt|cffffffffUiPlus|r |cff888888debug|r: " .. tostring(msg))
end
...
--
-- Every QtUiPlus event consumer goes through here so the ambiguity is handled
-- once instead of in each module.
-- ---------------------------------------------------------------------------
local listeners = {}   -- event -> array of callbacks
local dispatcher = CreateFrame("Frame", "QtUiPlusDispatcher", UIParent)

-- Records which handler shape actually arrived, so /qtp debug can report the
...
-- frames do not reliably receive OnUpdate ticks. QtUiPlus therefore runs one
...
-- itself again. Post-hooked native handlers run QtUiPlus's callback
...
U.RegisterEvent("ADDON_LOADED", function(event, addon)
  if type(addon) == "string" and string.lower(addon) == "qtuiplus" then
    Initialise()
  end
end)
