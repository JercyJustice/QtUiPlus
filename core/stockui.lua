...
-- client does not ship one. So every QtUiPlus reapply hook -- the Quest Log
-- font/strip passes and the Spellbook refresh -- was silently never installed,
-- which is why native updates were free to repaint quest text after QtUiPlus's
-- one-time pass. WORKING_SOURCE (UnrealPfUI), not runtime-verified.
--
-- QtUiPlus therefore owns the wrapper. The original is always called first and
...
local hookedGlobals = {}   -- global name -> array of QtUiPlus callbacks
...
-- Measured readout for /qtp check: whether this client provides its own
-- hooksecurefunc, and which globals QtUiPlus's wrapper actually replaced. The
...
function U.StockRegionKeep(frame, extra)
  local keep = {}
  if frame and frame.qtpFill then keep[frame.qtpFill] = true end
  if frame and type(frame.qtpEdges) == "table" then
    local i
    for i = 1, table.getn(frame.qtpEdges) do
      keep[frame.qtpEdges[i]] = true
    end
  end
...
--    A row that was empty when QtUiPlus first stripped it keeps alpha 0 after
...
function U.StripStockRowTextures(button, extra)
  ...
  -- Only the icons, never QtUiPlus's own fill/edge regions in `keep`.
  local i
...
-- back native black-on-parchment against QtUiPlus's dark panel, unreadable.
...
function U.StyleStockButton(button, options)
  ...
  if not button.qtpStockStyled then
    button.qtpStockStyled = true
...
-- while preserving the real spell icon and QtUiPlus's own border textures.
...
function U.StyleStockCloseButton(button, parent, x, y)
  ...
  local glyph = EnsureGlyph(button, "qtpCloseGlyph", "X", { 1, 0.25, 0.25, 1 },
                            M.fontSize.small)
  if glyph then
...
function U.StyleStockArrowButton(button, direction, size)
  ...
  EnsureGlyph(button, "qtpArrowGlyph", glyphs[key] or ">", M.color.text,
              M.fontSize.normal)
  return button
end
...
function U.StyleStockScrollbar(scrollbar)
  ...
  if not scrollbar.qtpTrack and up and down then
...
-- QtUiPlus does not hook. Rather than depend on that (or on an unverified
...
local function StyleGroupTab(button, options)
  if not button or button.qtpTabStyled then return button end
  button.qtpTabStyled = true
  ...
  local function Refresh()
    ...
    if ok and fontstring then
      U.SetStockFont(fontstring, options.fontSize or M.fontSize.small,
        button.qtpTabActive and M.color.textAccent or M.color.text)
    end
  end
  button.qtpTabRefresh = Refresh

  button.SetActive = function(active)
    button.qtpTabActive = active and true or false
    Refresh()
  end

  button:SetScript("OnEnter", function()
    if not button.qtpTabActive then
...
-- Adapts an existing native CheckButton to QtUiPlus's shared checkbox chrome.
...
function U.StyleStockCheckbox(button, sizeOrOptions)
  ...
  if not button.qtpCheckboxIndicatorStyled then
    button.qtpCheckboxIndicatorStyled = true
...
--    so the icon is a Button and carries the click itself. `qtpCollapseClick`
...
function U.StyleStockCollapseButton(button, expandedSize)
  if not button or button.qtpCollapseStyled then return button end
  button.qtpCollapseStyled = true
  ...
  local created, icon = pcall(CreateFrame, "Button",
    "QtUiPlusCollapseIcon" .. collapseIconCount, button)
  ...
  -- QtUiPlus's own hover feedback: the icon's border brightens to the addon
  ...
  -- be a FontString QtUiPlus creates and owns.
  ...
  icon:SetScript("OnClick", function()
    if type(button.qtpCollapseClick) == "function" then
      local ok, err = pcall(button.qtpCollapseClick, button)
  ...
  -- lit up beside QtUiPlus's icon instead of on it, since its native anchor
...
function U.SetStockCollapseState(button, shown, collapsed)
  local icon = button and button.qtpCollapseIcon
