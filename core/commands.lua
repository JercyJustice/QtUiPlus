-- QtUiPlus :: core/commands.lua
--
-- The /qtp slash command.
--
-- The settings interface is a later module. It registers itself by assigning
-- U.OpenSettings, and both /qtp and the minimap button will call through that
...
-- chat frame past what a player can usefully copy out. QtUiPlusDiagDB is a
-- second SavedVariables table, separate from QtUiPlusDB (core/config.lua),
-- used only as a write-out target for /qtp commands -- never read back by the
...
--
--   %AppData%\..\Local\Azeroth\Saved\Account\<account>\SavedVariables\QtUiPlus.lua
--
...
local function SaveDiagnostic(key, data)
  if type(QtUiPlusDiagDB) ~= "table" then QtUiPlusDiagDB = {} end
  QtUiPlusDiagDB[key] = data
  ...
  QtUiPlusDiagDB[key .. "_savedAt"] = stamp
end
...
-- other /qtp commands use rather than only ever printing to a chat frame the
...
function U.SavedVariablesHint()
  return "AppData/Local/Azeroth/Saved/Account/<account>/SavedVariables/QtUiPlus.lua"
end

-- Appends rather than overwrites, so several runs survive in one session.
-- SaveDiagnostic keys are single-slot: a second /qtp perf replaced the first,
...
function U.AppendDiagnostic(key, data)
  if type(QtUiPlusDiagDB) ~= "table" then QtUiPlusDiagDB = {} end
  if type(QtUiPlusDiagDB[key]) ~= "table" then QtUiPlusDiagDB[key] = {} end

  local list = QtUiPlusDiagDB[key]
...
local function ShowHelp()
  ...
  U.Print("  |cffffff00/qtp|r - open settings")
  U.Print("  |cffffff00/qtp unlock|r - unlock frames for moving")
  U.Print("  |cffffff00/qtp lock|r - lock frames")
  U.Print("  |cffffff00/qtp reset|r - reset all frame positions")
  U.Print("  |cffffff00/qtp bind|r - open quick binding")
  U.Print("  |cffffff00/qtp bindscan|r - dump the client binding table")
  U.Print("  |cffffff00/qtp keytest|r - measure whether key events reach addon frames")
  ...
  U.Print("  |cffffff00/qtp perf|r - record frame time around target changes")
...
-- frame past what is copyable, so it is written to QtUiPlusDiagDB.nameplates
...
local function ShowNameplateCheck()
  ...
  U.Print("  saved to QtUiPlusDiagDB.nameplates - |cffffff00/reload|r then " ..
          "open " .. U.SavedVariablesHint() .. " to read it")
end

-- Raw WorldFrame child dump, one line per child.
--
-- Nameplate detection is the one part of QtUiPlus with no compact-DB evidence
...
-- QtUiPlusDiagDB.nameplateDump and only a count prints in chat.
local function ShowNameplateDump()
  ...
  U.Print("WorldFrame children: " .. tostring(total) ..
          " - saved to QtUiPlusDiagDB.nameplateDump")
...
local ELITE_USAGE = "usage: |cffffff00/qtp elite|r " ..
...
local function HandleElite(rest)
  ...
  if sub == "tex" or sub == "texture" then
    ...
    U.Print("  no icon at all means the path does not render here - try " ..
            "another, |cffffff00/qtp elite tex default|r restores the built-in")
    return
  end
  ...
  U.Print("classification override: |cffffff00" .. (override or "off") ..
          "|r - target any mob; |cffffff00/qtp elite|r again for the next one")
  ShowEliteReport()
end

-- Reports what this client actually did with the calls QtUiPlus depends on.
...
local function ShowMoverTest()
  ...
  U.Print("Then |cffffff00/qtp check|r for measured drag state, " ..
          "|cffffff00/qtp lock|r, |cffffff00/reload|r, and " ..
          "|cffffff00/qtp movertest|r again to confirm it comes back in place.")
end
...
-- hovering. QtUiPlus also suppresses Lua errors by default, so a failing
...
local function ShowResistanceDump()
  ...
  local overlap = {
    ...
    "QtUiPlusCharacterModelRotateCatcher", "QtUiPlusCharacterPanel",
  ...
  table.insert(lines, "--- event trace (/qtp res trace) ---")
  ...
  U.Print("resistance dump: " .. tostring(table.getn(lines)) ..
          " lines saved to QtUiPlusDiagDB.resistances")
...
-- FireOnEnter invokes the very handlers this traces, so a /qtp res run would
...
local function StartEventTrace()
  ...
  U.Print("trace armed - hover the resistance icons, then a gear slot, " ..
          "then |cffffff00/qtp res|r and |cffffff00/reload|r")
end
  ...
  U.RegisterUpdate(resDump.HOVER_ID, 0.1, function(step)
    ...
    if elapsed >= resDump.HOVER_SECONDS then
      ...
      U.Print("hover watch done: " .. tostring(table.getn(samples)) ..
              " distinct targets saved to QtUiPlusDiagDB.resistanceHover")
...
--
-- QtUiPlus declares its own commands for bars 6-10 in Bindings.xml
...
local BINDING_SCAN_MARKERS = {
  -- QtUiPlus's own declarations (Bindings.xml).
  "QTUIPLUSBAR",
  ...
  -- entirely rather than rejecting QtUiPlus's file in particular.
  "PFPAGING",
  "PFSTANCE",
}
...
local function ShowBindingScan()
  ...
  U.Print("  addons loaded: QtUiPlus " .. AddOnLoaded("QtUiPlus") ..
  ...
  if type(getKey) == "function" then
    local got
    got, key1, key2 = pcall(getKey, "QTUIPLUSBAR2BUTTON1")
    if not got then key1, key2 = "error", nil end
  end
  U.Print("  GetBindingKey(QTUIPLUSBAR2BUTTON1): " .. tostring(key1) ..
          ", " .. tostring(key2))
  U.Print("  QtUiPlusActionButton: " .. type(U.G("QtUiPlusActionButton")))
end
...
-- QtUiPlusDiagDB because a busy quest window has more strings than chat can
-- usefully hold.
local function ShowQuestTextCheck()
  ...
  U.Print("  full dump saved to QtUiPlusDiagDB.questText")
end
...
local function StartKeyTest()
  ...
  local button = U.G("QtUiPlusKeyTestButton")
  local frame = U.G("QtUiPlusKeyTestFrame")

  if not button then
    button = CreateFrame("Button", "QtUiPlusKeyTestButton", UIParent)
    button:SetWidth(260)
    button:SetHeight(60)
    ...
    frame = CreateFrame("Frame", "QtUiPlusKeyTestFrame", UIParent)
    frame:SetWidth(260)
    frame:SetHeight(20)
    ...
    button.qtpLabel = U.CreateLabel(button, {
      size = U.media.fontSize.normal,
      color = U.media.color.accent,
      inherits = "GameFontNormal",
      width = 240,
    })
    if button.qtpLabel then
      button.qtpLabel:SetPoint("CENTER", button, "CENTER", 0, 0)
      button.qtpLabel:SetText("Key test: press keys. Escape or 15s stops it.")
    end
  end
  ...
  button:Show()
  frame:Show()
  if button.qtpLabel then button.qtpLabel:Show() end
...
--
-- Why this needs its own command rather than a /qtp perf switch: the perf
...
handlers["nosuppress"] = function()
  ...
  if U.db.noSuppress then
    ...
    U.Print("  the stock unit frames and action bars will be visible on top " ..
            "of QtUiPlus; that is expected, it is what suppression normally hides")
  else
...
handlers["suppress"] = function(rest)
  ...
  if not level then
    ...
    for i = 1, table.getn(SUPPRESS_LEVELS) do
      U.Print("  |cffffff00/qtp suppress " .. SUPPRESS_LEVELS[i])
    end
...
-- ---------------------------------------------------------------------------
-- Profiles (modules/profiles.lua)
--
-- The profile name keeps its original case: Split lowercases only the
-- subcommand it returns, not the remainder.
-- ---------------------------------------------------------------------------
...
if type(SlashCmdList) == "table" then
  SLASH_QTUIPLUS1 = "/qtp"
  SLASH_QTUIPLUS2 = "/qtuiplus"
  SlashCmdList["QTUIPLUS"] = function(message)
    if type(message) ~= "string" then message = U.G("arg1") end
    if type(message) ~= "string" then message = "" end
    HandleCommand(message)
  end
else
  U.Error("SlashCmdList is unavailable; /qtp not registered")
end
