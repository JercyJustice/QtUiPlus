...
-- ---------------------------------------------------------------------------
local textureColors = {}
local function MirrorTextureColor(texture, cached) texture.qtpColor = cached end
local function ReadTextureColor(texture) return texture.qtpColor end
...
-- QtUiPlus was not asked to suppress.
-- ---------------------------------------------------------------------------
...
-- cached, so /qtp suppress takes effect on the next sweep without a reload for
...
function U.SuppressNativeFrame(names, group)
  ...
  -- level, and registering regardless is what makes /qtp perf levels possible --
...
-- steady-state fast path. Used by /qtp perf levels, which raises the level in
...
-- Runtime self-check
--
-- Reports what this client actually did, so /qtp debug produces measured
