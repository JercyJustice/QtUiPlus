...
--
-- QtUiPlus therefore keeps SetBackdrop for the fill only and draws every
...
function U.CreateBorder(frame, thickness)
  ...
  if not frame.qtpEdges then
    ...
    frame.qtpEdges = edges
  end

  local i
  for i = 1, table.getn(EDGES) do
    if EDGES[i][3] == "horizontal" then
      frame.qtpEdges[i]:SetHeight(thickness)
    else
      frame.qtpEdges[i]:SetWidth(thickness)
    end
  end

  return frame.qtpEdges
end

function U.SetBorderColor(frame, r, g, b, a)
  if not frame or not frame.qtpEdges then return end
  local i
  for i = 1, table.getn(frame.qtpEdges) do
    U.SetColor(frame.qtpEdges[i], r, g, b, a)
  end
end
...
-- Applies the QtUiPlus panel look to an existing frame: flat fill plus the
...
function U.SetBackdropShown(frame, shown)
  ...
  if frame.SetBackdrop and frame.qtpFill == nil then
    if shown then
      pcall(frame.SetBackdropColor, frame, M.Unpack(M.color.background))
    else
      pcall(frame.SetBackdropColor, frame, 0, 0, 0, 0)
    end
  elseif frame.qtpFill then
    if shown then
      U.SetColor(frame.qtpFill, M.Unpack(M.color.background))
    else
      U.SetColor(frame.qtpFill, 0, 0, 0, 0)
    end
  end

  local i
  for i = 1, table.getn(frame.qtpEdges or {}) do
    if shown then frame.qtpEdges[i]:Show() else frame.qtpEdges[i]:Hide() end
  end
end
...
function U.SetBackgroundColor(frame, r, g, b, a)
  ...
  if frame.qtpFill then
    U.SetColor(frame.qtpFill, r, g, b, a)
    return
  end
...
-- Status bars
--
-- Not the client's StatusBar widget. QtUiPlus's first unit frames built one,
...
-- is a working implementation on this client, so QtUiPlus matches it -- and it
...
-- code. They are QtUiPlus's own functions on a plain Frame, not overrides of a
...
local function UpdateStatusBarFill(bar)
  ...
  local range = (bar.qtpMax or 0) - (bar.qtpMin or 0)
  local extent = 0
...
local function BarSetMinMaxValues(self, minimum, maximum)
  ...
  if self.qtpMin == minimum and self.qtpMax == maximum then return end
  self.qtpMin = minimum
  self.qtpMax = maximum
...
function U.CreateStatusBar(parent, options)
  ...
  bar.qtpBackground = bg

  local fill = bar:CreateTexture(nil, "ARTWORK")
  fill:SetTexture(M.texture.plain)
  U.SetColor(fill, M.Unpack(options.color or M.color.health))
  bar.qtpFillTexture = fill

  bar.qtpMin, bar.qtpMax, bar.qtpValue = 0, 1, 1
  bar.qtpVertical = (options.orientation == "VERTICAL")
...
-- QtUiPlus creates and owns rather than the Button's own text.
--
...
-- QtUiPlus creates the FontString (U.CreateButton) or reuses a native one
