...
-- at across reloads.
--
-- Everything is inert until /qtp perf turns it on: U.perfActive is a plain
...
local switches = {
  ...
  petbar  = true,  -- modules/petbar.lua     native bar anchor tick
...
--
-- Written on every /qtp perf report, independent of chat: a player does not
-- have to screenshot or hand-copy anything. |cffffff00/reload|r after running
-- a report flushes QtUiPlusDiagDB.perf to the client's SavedVariables file.
...
local function Report()
  ...
  -- both halves; QtUiPlusDiagDB.perf stays the latest run for convenience.
  local runIndex = nil
  ...
  U.Print("  saved to QtUiPlusDiagDB.perf" ..
  ...
  if count == 0 then
    U.Print("  no completed target-change samples yet - change target a few " ..
            "times, then |cffffff00/qtp perf|r again")
    return
  end
...
local function ShowUsage()
  U.Print("perf: |cffffff00/qtp perf|r - report, |cffffff00/qtp perf " ..
          "start|r / |cffffff00stop|r / |cffffff00reset|r")
  U.Print("  |cffffff00/qtp perf cycle|r - enable one subsystem at a time, " ..
          tostring(CYCLE_SECONDS) .. "s each, and compare them")
  U.Print("  |cffffff00/qtp perf levels|r - walk suppression level 0-4 in one " ..
          "run (needs |cffffff00/qtp suppress 0|r + reload first)")
  ...
  U.Print("    |cffffff00/qtp perf <name>|r - " .. line)
...
function U.PerfCommand(rest)
  ...
  if argument == "" then
    Report()
    if not active then
      U.Print("  |cffffff00/qtp perf start|r to begin recording")
    end
    return
  end
  ...
  if argument == "levels" then
    ...
    if level ~= 0 then
      U.Print("perf levels: needs to start with suppression off - run " ..
              "|cffffff00/qtp suppress 0|r then |cffffff00/reload|r, then this again")
      return
    end
    ...
    U.Print("  you do not need to read anything: |cffffff00/qtp perf|r then " ..
  ...
  if argument == "cycle" then
    ...
    U.Print("perf: |cff55ff55cycling|r - now |cffffff00move around and spam " ..
            ...
            "rotations, then |cffffff00/qtp perf|r")
    return
  end
