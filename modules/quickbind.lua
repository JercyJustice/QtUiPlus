-- QtUiPlus :: modules/quickbind.lua
--
...
-- General Options, and from /qtp bind.
--
...
--     reproduced here) and QtUiPlus's own chat wheel remap (knowledge.json /
...
--     game on 2026-08-19 with /qtp keytest): on this client a plain Frame with
...
-- Green rather than the QtUiPlus accent so it cannot be mistaken for the
...
-- Only reachable when the client has not registered QtUiPlus's declared
...
local function ShowTooltip(catcher)
  ...
  -- Only reachable if a client ever registers QtUiPlus's declared commands:
  -- they are filed under an QtUiPlus header rather than beside the stock keys.
  if catcher.declared then
    pcall(tooltip.AddLine, tooltip,
          "Key Bindings: QtUiPlus Bar " .. tostring(catcher.bar),
          0.96, 0.68, 0.04)
  end
...
local function CreateShade()
  ...
  shade = CreateFrame("Button", "QtUiPlusQuickBindShade", UIParent)
...
--
-- Measured on this client (/qtp keytest, 2026-08-19): a plain Frame with
...
local function CreateKeyCatcher()
  keys = CreateFrame("Frame", "QtUiPlusQuickBindKeys", UIParent)
