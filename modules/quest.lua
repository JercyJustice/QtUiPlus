...
-- data, item slots and click behaviour stay intact; QtUiPlus changes only
...
-- options opens QuestFrame's greeting panel directly -- and QtUiPlus skinned
...
-- captured by the live /qtp questtext diagnostic on this client; the shorter
...
-- These exact detail/reward heading names were identified by the live /qtp
...
-- The live /qtp questtext dump exposed QuestRewardItem1..9 on this client.
...
-- neuter those textures' Show methods outright; QtUiPlus instead re-runs the
...
local function StyleTitleRows()
  ...
  for i = 1, rows do
    ...
    if button then
      ...
      ForceRowWhite()
      if not button.qtpQuestWhiteTextHooks then
        button.qtpQuestWhiteTextHooks = true
...
-- Creating an QtUiPlus backdrop on the container therefore produced the two
  ...
  for p = 1, table.getn(ITEM_PREFIXES) do
    ...
    for i = 1, ITEMS_PER_PANEL do
      ...
      if button then
        ...
        -- QtUiPlus panel around the real icon/name content instead.
        if p > 1 and icon and itemName then
          local card = button.qtpQuestRewardCard
          if not card then
            local owner = frame
            ...
            if created and owned then
              card = owned
              button.qtpQuestRewardCard = card
              ...
              if not button.qtpQuestRewardCardHooks then
                button.qtpQuestRewardCardHooks = true
                U.PostHookScript(button, "OnEnter", function()
                  if button.qtpQuestRewardCard then
                    U.SetBorderColor(button.qtpQuestRewardCard,
...
--
-- Read via /qtp questtext with the NPC window open. Anything the walker never
