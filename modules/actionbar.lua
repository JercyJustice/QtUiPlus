-- QtUiPlus :: modules/actionbar.lua
--
...
-- QtUiPlus neither displays nor installs a key route for those buttons.
...
-- The commands QtUiPlus declares for itself in Bindings.xml, covering the pages
...
local DECLARED_PREFIX = {
  [6]  = "QTUIPLUSBAR6BUTTON",
  [7]  = "QTUIPLUSBAR7BUTTON",
  [8]  = "QTUIPLUSBAR8BUTTON",
  [9]  = "QTUIPLUSBAR9BUTTON",
  [10] = "QTUIPLUSBAR10BUTTON",
}
...
-- The two globals QtUiPlus *does* post-hook, ActionButtonDown/Up, are read
...
--
--   * SetOverrideBindingClick -- absent on this build (/qtp check).
--   * Bindings.xml declarations -- QtUiPlus's 60 commands were absent from a
--     225-entry binding table (/qtp bindscan). The file is still shipped and
...
local firstDeclaredCommand = "QTUIPLUSBAR2BUTTON1"
...
-- The binding command for a slot, native or QtUiPlus-declared. nil means this
...
-- unassigned key therefore cannot remain attached to a stale QtUiPlus slot.
--
      ...
      for index = 1, table.getn(entry.buttons) do
        ...
        elseif prefix then
          ...
          if type(key1) == "string" and key1 ~= "" then
            if not pcall(bind, button, false, key1, button.qtpName, "LeftButton") then
              clean = false
            end
          end
          if type(key2) == "string" and key2 ~= "" and key2 ~= key1 then
            if not pcall(bind, button, false, key2, button.qtpName, "LeftButton") then
              clean = false
            end
          end
        end
...
-- SetOverrideBindingClick targets the named QtUiPlus button, so keyboard and
...
local function ShowButtonPress(button, held)
  if not button.qtpPressed then return end

  local now = tonumber(Call("GetTime"))
  button.qtpPressedUntil = now and (now + PRESS_FLASH_DURATION) or nil
  button.qtpPressedHeld = held and true or nil
  button.qtpPressedShown = true
  button.qtpPressed:Show()
  ApplyButtonBorder(button)

  if not button.qtpPressedTracked then
    button.qtpPressedTracked = true
    table.insert(pressedButtons, button)
  end
end

local function ReleaseButtonPress(button)
  if button then button.qtpPressedHeld = nil end
end
...
local function RefreshPressedButtons()
  ...
  for i = table.getn(pressedButtons), 1, -1 do
    ...
    if not button.qtpPressedHeld and
       (not now or not button.qtpPressedUntil or now >= button.qtpPressedUntil) then
      if button.qtpPressed then button.qtpPressed:Hide() end
      button.qtpPressedUntil = nil
      button.qtpPressedTracked = nil
      button.qtpPressedShown = nil
      ApplyButtonBorder(button)
...
-- a successful action does not imply that QtUiPlus's OnClick ran. Post-hook the
...
-- the key to that frame's fixed/native action rather than to QtUiPlus's
...
local function InstallDeclaredBindingHandler()
  local handler = function(bar, index)
    ...
    if button and button.qtpShown then OnButtonClick(button) end
  end

  U.SetG("QtUiPlusActionButton", handler)
  return U.G("QtUiPlusActionButton") == handler
end
...
local function InstallDeclaredBindingNames()
  ...
  for bar, prefix in pairs(DECLARED_PREFIX) do
    U.SetG("BINDING_HEADER_QTUIPLUSBAR" .. bar, "QtUiPlus Bar " .. bar)
    local i
...
local function CreateButton(bar, index)
  local name = "QtUiPlusActionBar" .. bar .. "Button" .. index
  ...
  button.qtpCooldownLayer = textLayer
  ...
  -- RefreshGCDSweep.
  button.qtpGcd = U.CreateRadialWipe(textLayer)

  button.qtpCooldownText = U.CreateLabel(textLayer, {
    size = M.fontSize.normal, color = CD_COLOR.normal,
    inherits = "GameFontNormal",
  })
  if button.qtpCooldownText then
    button.qtpCooldownText:SetPoint("CENTER", textLayer, "CENTER", 0, 0)
    button.qtpCooldownText:Hide()
  end
...
local function SizeButton(button, size)
  ...
  if button.qtpCount then
    ...
    button.qtpCount:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", -2, 2)
    U.SetFont(button.qtpCount, small)
  end
  if button.qtpMacro then
    button.qtpMacro:ClearAllPoints()
    button.qtpMacro:SetPoint("BOTTOMLEFT", button, "BOTTOMLEFT", 2, 2)
    button.qtpMacro:SetWidth(size - 6)
    U.SetFont(button.qtpMacro, small)
  end
  U.SizeRadialWipe(button.qtpGcd, size)
  ...
  if button.qtpCooldownText then
    ...
    U.SetFont(button.qtpCooldownText, cdSize)
  end
end
...
local function HideButton(button)
  ...
  ShowRegion(button.qtpCooldownText, false)
  ShowRegion(button.qtpPressed, false)
  button.qtpCdActive = false
  button.qtpCdShown = false
  if button.qtpCooldown then pcall(button.qtpCooldown.Hide, button.qtpCooldown) end
  U.HideRadialWipe(button.qtpGcd)
  button:Hide()
end
...
local function UpdateSlot(button)
  ...
  if type(texture) == "string" and texture ~= "" then
    ...
    U.SetColor(button.qtpIcon, 1, 1, 1, 1)
    button.qtpIcon:Show()
    button.qtpEmpty = false
  ...
  if button.qtpCount then
    button.qtpCount:SetText(count)
    ShowRegion(button.qtpCount, count ~= "")
  end
  ...
  if button.qtpMacro then
    if type(macro) == "string" and macro ~= "" then
      button.qtpMacro:SetText(macro)
      button.qtpMacro:Show()
    else
      button.qtpMacro:SetText("")
      button.qtpMacro:Hide()
    end
  end

  local key = ""
  if cfg.showKeybind then key = BindingFor(button.qtpBar, button.qtpIndex) end
  if button.qtpKeybind then
    button.qtpKeybind:SetText(key)
    ShowRegion(button.qtpKeybind, key ~= "")
  end
end

local function UpdateUsable(button)
  local slot = ButtonSlot(button)
  if button.qtpEmpty then return end
  ...
  -- Reads button.qtpCdActive, which UpdateCooldown must therefore set before
  ...
  -- (button.qtpCdActive is false for those), so the icon does not flash red on
  ...
  if button.qtpTint ~= color then
    button.qtpTint = color
    U.SetColor(button.qtpIcon, color[1], color[2], color[3], color[4])
  end
end
...
local function UpdateActive(button)
  ...
  if active == button.qtpActive then return end
  button.qtpActive = active
  ApplyButtonBorder(button)
end
...
local function RefreshCooldownText(button)
  local label = button.qtpCooldownText
  if not label then return end

  local remaining = nil
  if button.qtpCdActive and cfg and cfg.showCooldown then
    remaining = CooldownRemaining(button.qtpCdStart, button.qtpCdDuration)
  end

  if not remaining or remaining <= 0 then
    if remaining and remaining <= 0 then button.qtpCdActive = false end
    if button.qtpCdShown then
      button.qtpCdShown = false
      button.qtpCdColor = nil
      label:SetText("")
      label:Hide()
    end
    return
  ...
  if button.qtpCdColor ~= color then
    button.qtpCdColor = color
    pcall(label.SetTextColor, label, color[1], color[2], color[3], color[4])
  end
  if not button.qtpCdShown then
    button.qtpCdShown = true
    label:Show()
  end
end
...
local function UpdateCooldown(button)
  ...
  if button.qtpCooldown then
    ...
    if fn then
      pcall(fn, button.qtpCooldown, start, duration, enable or 1)
    end
  end
  ...
  button.qtpCdActive = (start > 0 and duration >= GCD_THRESHOLD
...
local function ApplyGCDSweep(button)
  ...
  if button.qtpEmpty then return end
  U.SetRadialWipeProgress(button.qtpGcd, gcdProgress)
end
...
local function CreateBar(bar)
  local frame = CreateFrame("Frame", "QtUiPlusActionBar" .. bar, UIParent)
...
local function LayoutBar(bar)
  ...
  for i = 1, SLOTS_PER_BAR do
    ...
    if enabled and i <= count then
      ...
      button:Show()
      button.qtpShown = true
      button.qtpTint = nil
      button.qtpActive = nil
      FullUpdate(button)
    else
      button.qtpShown = false
      HideButton(button)
    end
  end
...
-- either way, and /qtp perf bars prices what is left.
local STATE_EVENTS = {
...
-- `declared` marks the slots whose command comes from QtUiPlus's own
-- Bindings.xml rather than from the client, which is worth telling the player:
-- the key is real and saved, but it is listed under an QtUiPlus header in the
...
function U.ActionBarBindTargets()
  ...
  ForEachVisibleButton(function(button)
    local bar = button.qtpBar
    table.insert(targets, {
      button = button,
      bar = bar,
      index = button.qtpIndex,
      command = U.ActionBindingCommand(bar, button.qtpIndex),
...
-- Whether the client registered QtUiPlus's own binding commands. False means
...
-- Kept as a capability readout for /qtp check. This client (build 5875) is not
...
function AB:OnEnable()
  ...
  -- main route they wrap QtUiPlus's replacement, never the hidden stock path.
  ...
  for i = 1, BAR_COUNT do
    ...
    table.insert(report, {
      ...
      cooldownFrame = (entry and entry.buttons[1] and
                       entry.buttons[1].qtpCooldown) and true or false,
      cooldownText = (entry and entry.buttons[1] and
                      entry.buttons[1].qtpCooldownText) and true or false,
      gcdSweep = (entry and entry.buttons[1] and
                  entry.buttons[1].qtpGcd) and true or false,
    })
  end
  return report
end
