...
-- filter and buy/cancel behaviour stay intact; QtUiPlus changes only
...
local function NudgeRowText(row)
  ...
  if not row or row.qtpRowTextNudged or not row.GetFontString then return end
...
-- its click routing are QtUiPlus's, same treatment as modules/questlog.lua's
-- header rows.
...
-- was still fully native (parchment art, no QtUiPlus styling at all) after
