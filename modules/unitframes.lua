-- QtUiPlus :: modules/unitframes.lua
--
...
-- local because QtUiPlus does not install shim globals.
...
-- literals: /qtp elite tex|size|coord answers both questions in one session
...
-- /qtp elite's test override. Forces the classification every frame reads, so
...
local function ReadIdentity(frame)
  ...
  -- level-text "+"/"R+" suffix and the icon, is what keeps the /qtp elite
...
-- the class read.
--
-- The indicator is QtUiPlus's own flat swatch rather than the Blizzard face:
...
-- Second attempt was QtUiPlus's own flat accent glyph, no client asset at all.
...
local function BuildFrame(spec, parent)
  ...
  local frame = CreateFrame("Button", "QtUiPlusUnit" .. spec.name, parent or UIParent)
...
local function SetBar(bar, value, maximum)
  ...
  if bar.qtpMin ~= 0 or bar.qtpMax ~= maximum then
    pcall(bar.SetMinMaxValues, bar, 0, maximum)
  end
  if bar.qtpValue ~= value then
...
-- QtUI "Gradient bars": a mild left-to-right fade of the bar's own colour,
-- clipped (not squeezed) as health drops. Not a red/yellow/green HP mix.
...
-- no GetVertexColor to read one back with, so /qtp elite status can still
...
local function SetFrameShown(frame, shown)
  if frame.qtpShown == shown then return end
  frame.qtpShown = shown
  if shown then frame:Show() else frame:Hide() end
end

local function ApplyConnectedAlpha(frame)
  local alpha = frame.data.connected and 1 or 0.35
  if frame.qtpAlpha == alpha then return end
  if pcall(frame.SetAlpha, frame, alpha) then frame.qtpAlpha = alpha end
end
...
-- /qtp check prints the raw returns -- the readout that turns this gap into
...
local druidRaw = {}                  -- last measured values, for /qtp check
...
local function ShowDruidBar(shown)
  if not druidBar or druidBar.qtpShown == shown then return end
  druidBar.qtpShown = shown
...
-- nil for every class except druid, so /qtp check stays silent where the bar
-- does not exist.
...
-- which is the same client level QtUiPlus targets (Interface: 11200):
...
local function BuildPartyAnchor()
  ...
  local anchor = CreateFrame("Frame", "QtUiPlusPartyAnchor", UIParent)
...
local function BuildUnitFrameColorSettings(parent, y)
  ...
  local customToggle = U.CreateCheckbox(parent, {
    name = "QtUiPlusSettingsCustomColors",
  ...
  for i = 1, table.getn(POWER_TYPES) do
    ...
    local picker = U.CreateColorPicker(parent, {
      name = "QtUiPlusSettingsPowerColor" .. entry.label,
      text = entry.label,
...
-- Reports what the client actually did with the unit API, so /qtp check can
...
function U.UnitFrameReport()
  ...
  for i = 1, table.getn(frameOrder) do
    ...
    --
    -- The bars are QtUiPlus's own (core/style.lua) after the client's StatusBar
    ...
    line.barValue = bar.qtpValue
    line.barMin, line.barMax = bar.qtpMin, bar.qtpMax
...
-- ---------------------------------------------------------------------------
-- Elite icon test surface (/qtp elite)
--
