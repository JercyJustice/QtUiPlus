-- QtUiPlus :: modules/gamemenu.lua
--
-- QtUiPlus-owned presentation for the native GameMenuFrame (Escape menu).
...
local function StyleNativeButton(button)
  ...
  local cover = button.qtpGameMenuCover
  if not cover then
...
local function EnsureOwnButtons(frame)
  if not qtpButton then
    qtpButton = U.CreateButton(frame, {
      name = "QtUiPlusGameMenuButton",
      text = "|cffffffffQt|cff33c0ffUiPlus|r",
      width = BUTTON_WIDTH,
    ...
    RaiseAbove(qtpButton, frame, 20)
  end

  -- Quick binding sits directly under the QtUiPlus row: it is a mode, not a
  ...
  if not bindButton then
    bindButton = U.CreateButton(frame, {
      name = "QtUiPlusGameMenuQuickBindButton",
      text = "Quick Binding",
      width = BUTTON_WIDTH,
      height = BUTTON_HEIGHT,
      background = { 0.025, 0.025, 0.025, 0.96 },
  ...
  if not closeButton then
    closeButton = U.CreateButton(frame, {
      name = "QtUiPlusGameMenuCloseButton",
      text = "Close",
      width = BUTTON_WIDTH,
      height = BUTTON_HEIGHT,
      background = { 0.025, 0.025, 0.025, 0.96 },
...
local function CaptureOrder(frame, buttons)
  ...
  for i = 1, table.getn(buttons) do
    ...
    if button ~= continueBtn and button ~= qtpButton and
  ...
  -- Replaced by the QtUiPlus-owned Close row above.
...
local function Layout(frame)
  ...
  for i = 1, table.getn(order) do
    ...
    if shown then
      if button ~= qtpButton and button ~= bindButton and
