...
-- scrolling, row templates and click behaviour stay intact; QtUiPlus changes
...
-- native *_Update runs QtUiPlus's callback synchronously, inside the same
...
-- finish and settle before QtUiPlus touches the same rows.
--
...
local function DisableRemoveFriend(button)
  ...
  local shield = button.qtpRemoveFriendShield
  if not shield then
    shield = U.CreateButton(frame or UIParent, {
      name = "QtUiPlusRemoveFriendShield",
      text = "",
      width = 1,
      height = 1,
    })
    ...
    button.qtpRemoveFriendShield = shield
  end
...
local function EnsureWhoSearchBackdrop(scroll)
  ...
  if not whoSearchBackdrop then
    whoSearchBackdrop = U.CreatePanel(scroll, {
      name = "QtUiPlusWhoSearchBackdrop",
      width = 1,
      height = 1,
    })
