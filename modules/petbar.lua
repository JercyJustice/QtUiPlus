-- QtUiPlus :: modules/petbar.lua
--
-- The native pet action bar, left exactly as the client draws it, with one
-- QtUiPlus addition: a mover handle so it can be placed like the rest of the
-- interface.
--
-- Why there is no QtUiPlus pet bar any more
--
--   CastPetAction -- the call this client uses to activate a pet action -- is
--   documented as protected: addons cannot call it, and the call errors. The
--   unprotected substitutes cover the command and stance slots only:
...
--   Sacrifice and a hunter's pet abilities live.
--
--   A custom pet bar on this client therefore cannot cast them at all. The
--   previous revision of this module built one anyway -- it called
--   CastPetAction directly -- and made the situation worse by also suppressing
--   the native bar that does work.
--
--   That is a client capability limit, not a bug with a workaround, so the
--   feature is gone rather than emulated.
--
-- What is left
--
--   The native bar is never hidden, reskinned or click-handled. A QtUiPlus-
...
--
--   * The handle lives on a QtUiPlus frame rather than on PetActionBarFrame
--     directly. The mover handle is SetAllPoints to the frame it is registered
--     on, and this client's pet bar reports no dependable size for that to
--     cover. An owned frame has a size we set, so the handle is always there
--     to grab.
--   * The native bar's own anchor is not UIParent-relative, so it cannot be
--     expressed as a mover `default`. core/mover.lua's U.OnPositionReset is
--     the route for exactly that case; the anchor read at load is replayed
--     from there so /qtp reset really does put the bar back where the client
--     had it.
--   * Anchors are read through U.GetFramePoint, which hands back values in the
--     shape SetPoint expects. The captured native anchor is replayed with
--     those values unchanged.
--   * UNIT_PET and the PET_BAR_* events have no compact record on this client,
--     so they are accelerators only. The slow shared updater is the guarantee.
...
-- of ours can have moved the bar. Replayed on /qtp reset. U.GetFramePoint
-- already returns the relative frame resolved and Y in the sign SetPoint
-- wants, so the capture goes straight back through SetPoint unchanged.
-- ---------------------------------------------------------------------------
...
local function MirrorNativeSize()
  ...
  if anchor.qtpWidth ~= width then
    anchor:SetWidth(width)
    anchor.qtpWidth = width
  end
  if anchor.qtpHeight ~= height then
    anchor:SetHeight(height)
    anchor.qtpHeight = height
  end
end
...
--   MainMenuBar is the first name in its root list: the root is hidden,
--   alpha zeroed, mouse-disabled and has its Show replaced with a no-op.
--   Vanilla parents PetActionBarFrame into that hierarchy, and a hidden parent
--   takes every descendant down with it. The result is a mover handle that is
--   there and grabbable, a client that still shows and updates the bar,
--   PetHasActionBar true -- and nothing drawn, because an ancestor is hidden.
--
...
--   still the only thing on this client that can cast a pet spell.
--
-- What is checked rather than assumed
--
--   This client has no runtime record of the pet bar at all, so the parent is
--   *read* and the move only happens when it is not already UIParent. A client
--   that parents its pet bar somewhere safe is left completely alone.
--
--   Scale multiplies down the parent chain here, so a parent carrying a scale
--   of its own would resize the bar the moment it moved. The effective scale
--   is measured before the move and restored after it.
--
...
local function Apply()
  ...
  if not position then
    -- Never placed, or /qtp reset: give the bar back to the client once, then
...
local function CreateAnchor()
  anchor = CreateFrame("Frame", "QtUiPlusPetBarAnchor", UIParent)
...
function PB:OnEnable()
  ...
  U.RegisterMover("petbar", anchor, { label = "Pet Bar" })
