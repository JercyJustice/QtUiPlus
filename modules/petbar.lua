...
local function ApplyBorder(button)
  ...
  elseif button.qtpAutocast and cfg and cfg.showAutocast then
...
local function OnButtonClick(button, a, b)
  ...
  if CursorHoldsAction() then
    Call("PickupPetAction", button.qtpIndex)
    return
  end

  if mouseButton == "RightButton" then
    Call("TogglePetAutocast", button.qtpIndex)
    return
  end

  if Call("IsPetAttackActive", button.qtpIndex) then
    Call("PetStopAttack")
  else
    Call("CastPetAction", button.qtpIndex)
  end
end
...
local function ShowTooltip(button)
  ...
  local name, _, _, token = GetPetInfo(button.qtpIndex)
  ...
  elseif type(tooltip.SetPetAction) == "function" then
    if not pcall(tooltip.SetPetAction, tooltip, button.qtpIndex) then
...
local function CreateButton(index)
  local name = "QtUiPlusPetBarButton" .. index
...
local function SizeButton(button, size)
  ...
  button.qtpIcon:ClearAllPoints()
  button.qtpIcon:SetPoint("TOPLEFT", button, "TOPLEFT", ICON_INSET, -ICON_INSET)
  button.qtpIcon:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", -ICON_INSET, ICON_INSET)
end

local function HideButton(button)
  if button.qtpIcon then button.qtpIcon:Hide() end
...
local function UpdateSlot(button)
  ...
  pcall(button.qtpIcon.SetDesaturated, button.qtpIcon, not usable)

  button.qtpActive = active and true or false
  button.qtpAutocast = autocast and true or false
  ApplyBorder(button)
end

local function UpdateCooldown(button)
  local start, duration, enable = Call("GetPetActionCooldown", button.qtpIndex)
  ...
  if button.qtpCooldown then
    ...
    if fn then
      pcall(fn, button.qtpCooldown, start, duration, tonumber(enable) or 1)
    end
  end
...
local function ForEachButton(callback)
  -- /qtp perf petbar. Both recurring sweeps walk the buttons through here.
...
local function CreateBar()
  frame = CreateFrame("Frame", "QtUiPlusPetBar", UIParent)
...
local function BuildSettingsPage(parent)
  ...
  local enable = U.CreateCheckbox(parent, {
    name = "QtUiPlusPetBarConfigEnable",
    text = "Enable",
  ...
  local autocast = U.CreateCheckbox(parent, {
    name = "QtUiPlusPetBarConfigAutocast",
  ...
  for i = 1, table.getn(SLIDERS) do
    ...
    local slider = U.CreateSlider(parent, {
      name = "QtUiPlusPetBarConfig" .. spec.key,
      text = spec.text,
      width = 200,
