...
-- separate QtUiPlus window (modules/bank.lua) built on the same shared slot
...
local function BuildMoneyDisplay(parent)
  local money = CreateFrame("Frame", "QtUiPlusBagMoney", parent)
  money:SetHeight(16)
  money:SetWidth(150)
...
local function EnsureSlot(bag, slot, parent)
  ...
  local name = "QtUiPlusBagSlot" .. (bag < 0 and ("m" .. -bag) or bag) .. "_" .. slot
...
-- bags has to land on QtUiPlus's merged frame instead of the native
...
local function InstallToggleOverrides()
  if U.qtpBagTogglesOverridden then return end
  U.qtpBagTogglesOverridden = true
...
local function Build()
  ...
  anchor = CreateFrame("Frame", "QtUiPlusBagAnchor", UIParent)
  ...
  frame = U.CreatePanel(anchor, { name = "QtUiPlusBagFrame" })
  ...
  if type(special) == "table" then
    table.insert(special, "QtUiPlusBagFrame")
  end

  BuildHeader()

  grid = CreateFrame("Frame", "QtUiPlusBagGrid", frame)
  ...
  frame.keyring = BuildTray("QtUiPlusBagKeyring")
  frame.keyring:SetPoint("BOTTOMLEFT", frame, "TOPLEFT", 0, SLOT_GAP)

  frame.bagslots = BuildTray("QtUiPlusBagSlots")
  ...
  -- windows back over QtUiPlus's frame. Uses the shared native-suppression
