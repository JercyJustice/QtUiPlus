...
-- intact; QtUiPlus changes only artwork, typography and layout.
...
local QUEST_ROWS = 23
-- Quest log headings use QtUiPlus's real chrome accent (#f5ae0a) rather than
...
-- Filled by the first header click attempt so /qtp check can report which of
...
-- behaviour. So QtUiPlus can re-establish tracking itself: remember which
...
local function SetDetailVisible(show)
  ...
  detail.qtpUserHidden = show and nil or true
  if show then
  ...
  local expand = G("QtUiPlusQuestLogExpand")
...
local function BuildQuestLevelToggle(collapseAll)
  ...
  local ok, toggle = pcall(CreateFrame, "CheckButton",
    "QtUiPlusQuestLogLevels", frame, "UICheckButtonTemplate")
  ...
  local text = G("QtUiPlusQuestLogLevelsText")
  if text then
    text:SetText("Levels")
...
-- actually available, so /qtp check turns the first click into evidence instead
-- of another round of blind iteration.
local function ToggleHeader(row)
  local index = row and row.qtpQuestIndex
...
--
-- So QtUiPlus stops trying to place the native texture and owns the mark: a
...
-- client keeps deciding what is tracked and QtUiPlus only renders it.
local TRACK_MARK_WIDTH = 4

local function BuildTrackMark(row)
  if not row or row.qtpTrackMark then return row.qtpTrackMark end

  local heightOk, height = pcall(row.GetHeight, row)
  local barHeight = heightOk and tonumber(height) and math.max(height - 6, 6) or 10

  -- USER_CONFIRMED_INGAME: the mark needs QtUiPlus's actual chrome colour
...
local function BuildRows()
  ...
  for i = 1, QUEST_ROWS do
    ...
    if row then
      row.qtpCollapseClick = ToggleHeader
...
local function UpdateRows()
  ...
  if detail and detail.qtpUserHidden then pcall(detail.Hide, detail) end
  ...
  for i = 1, QUEST_ROWS do
    ...
    if row then
      ...
      row.qtpQuestIndex = titleOk and isHeader and questIndex or nil
      row.qtpCollapsed = isCollapsed and true or false
      ...
      -- Which one answers is recorded so /qtp check can say whether the mark is
  ...
  -- The All button keeps its native click; only its icon is QtUiPlus's, so its
...
local function StyleQuestItems()
  ...
  for i = 1, maxItems do
    ...
    if item and not item.qtpQuestItemStyled then
      item.qtpQuestItemStyled = true
...
-- player moves it; unlike QtUiPlus-owned overlays, it has no fabricated
...
local function BuildFrame()
  ...
  U.PostHookScript(detail, "OnHide", function()
    ...
    if frame and detail.qtpUserHidden then
      pcall(frame.SetWidth, frame, 340)
  ...
  U.PostHookScript(detail, "OnShow", function()
    ...
    if not detail.qtpUserHidden then
      pcall(frame.SetWidth, frame, 676)
      U.StyleStockArrowButton(expand, "left", 21)
    end
  end)
  ...
  -- No qtpCollapseClick override: the All button's native OnClick already works
  ...
  local listPanel = U.CreatePanel(frame, {
    name = "QtUiPlusQuestLogListPanel",
  ...
  detailPanel = U.CreatePanel(frame, {
    name = "QtUiPlusQuestLogDetailPanel",
    background = { 0.01, 0.01, 0.01, 0.74 },
  })
  ...
  if empty then
    ...
    -- the normal QtUiPlus list panel remains the only empty-state surface.
