-- QtUiPlus :: modules/bank.lua
--
-- The bank window, in QtUiPlus's modern style: one flat near-black panel with
...
-- hiding it would end the banker session the moment QtUiPlus's window opened;
...
local function EnsureSlot(bag, slot, parent)
  ...
  local name = "QtUiPlusBankSlot" .. (bag < 0 and ("m" .. -bag) or bag) ..
               "_" .. slot
...
local function EnsureBankBagButton(index)
  ...
  local name = "QtUiPlusBankBag" .. index
  ...
  button.qtpBag = index + BANK_BAG_OFFSET
  ...
  button:SetScript("OnEnter", function()
    ...
    HighlightBag(button.qtpBag, true)
  ...
  button:SetScript("OnLeave", function()
    ...
    HighlightBag(button.qtpBag, false)
    local tip = U.G("GameTooltip")
...
local function BuildHeader()
  ...
  frame.bags = CreateFrame("Frame", "QtUiPlusBankBags", frame)
...
local function BuildDragHandle()
  local handle = CreateFrame("Button", "QtUiPlusBankDrag", frame)
...
local function Build()
  ...
  anchor = CreateFrame("Frame", "QtUiPlusBankAnchor", UIParent)
  anchor:SetWidth(COLUMNS * (SLOT_SIZE + SLOT_GAP) - SLOT_GAP + PADDING * 2)
  ...
  if type(special) == "table" then
    table.insert(special, "QtUiPlusBankFrame")
  end

  BuildHeader()
  BuildDragHandle()

  grid = CreateFrame("Frame", "QtUiPlusBankGrid", frame)
