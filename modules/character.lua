-- QtUiPlus :: modules/character.lua
--
...
-- intact; QtUiPlus changes only artwork, typography and layout, matching the
...
--
-- Measured on this client with /qtp res (QtUiPlusDiagDB.resistances,
...
--
-- So QtUiPlus reuses the native region rather than owning one: it is kept out
...
local function FindResistIcon(res)
  if res.qtpResistIcon then return res.qtpResistIcon end
  ...
  for i = 1, table.getn(regions) do
    ...
    if typeOk and objectType == "Texture" and region.GetName then
      ...
      if nameOk and type(name) == "string" and
        ...
        res.qtpResistIconPath = string.gsub(name, "_0[xX]%x+$", "")
        return region
      end
    end
  end
...
-- correctly localized, so QtUiPlus does not replace it -- it only makes sure
...
local function StyleResistance(res, index)
  ...
  if icon then
    if res.qtpResistIconPath then
      pcall(icon.SetTexture, icon, res.qtpResistIconPath)
    end
    ApplyResistIconCrop(icon, index)

    -- Inset by the outline thickness so QtUiPlus's own 1-unit border stays the
...
-- The resistance column overlaps CharacterModelFrame, and QtUiPlus's own
...
-- hover. /qtp res proved the native OnEnter itself is fine: called directly it
...
-- resistances are not.
-- Measured overlap (/qtp res, USER_CONFIRMED_INGAME): QtUiPlus's click-rotate
...
local function RaiseResistancesAboveModel()
  local catcher = G("QtUiPlusCharacterModelRotateCatcher")
...
-- reading the undocumented `this`), this follows QtUiPlus's own verified
...
local function StyleModel()
  ...
  local created, catcher = pcall(CreateFrame, "Button",
    "QtUiPlusCharacterModelRotateCatcher", model)
...
local function ToggleReputationHeader(header)
  local index = header and header.qtpReputationIndex
  if not index then return end

  local fn = G(header.qtpCollapsed and "ExpandFactionHeader" or
...
UpdateReputationRows = function()
  ...
  for i = 1, REP_ROWS do
    ...
    if header then
      ...
      header.qtpReputationIndex = infoOk and isHeader and (i + offset) or nil
      header.qtpCollapsed = infoOk and isHeader and isCollapsed and true or false
...
StyleReputationBar = function(bar)
  ...
  -- SetStatusBarTexture retargets the live fill to QtUiPlus's plain texture.
...
local function StyleReputationTab()
  ...
  for i = 1, count do
    ...
    local header = G("ReputationHeader" .. i)
    if header then
      header.qtpCollapseClick = ToggleReputationHeader
...
local function ToggleSkillHeader(row)
  local index = row and row.qtpSkillIndex
  if not index then return end

  local collapsed = row.qtpCollapsed
  ...
  -- Always resync QtUiPlus's own icons/bars after a toggle, regardless of
...
local function BuildSkillRows()
  ...
  for i = 1, SKILL_ROWS do
    ...
    if header then
      header.qtpCollapseClick = ToggleSkillHeader
      U.StyleStockCollapseButton(header)
    end
  end
end
...
UpdateSkillRows = function()
  ...
  for i = 1, SKILL_ROWS do
    ...
    if header then
      header.qtpSkillIndex = infoOk and isHeader and index or nil
      header.qtpCollapsed = infoOk and isHeader and not isExpanded
      U.SetStockCollapseState(header, infoOk and isHeader, header.qtpCollapsed)
      SetTextFont(header, M.fontSize.normal, GOLD)
    end
    if bar then
...
local function StyleSkillsTab()
  ...
  if collapseAll then
    ...
    -- QtUiPlus keeps only the icon's cosmetic glyph and moves hover feedback
    -- onto the real button.
    local icon = collapseAll.qtpCollapseIcon
    if icon then
...
--
-- So the glyph is re-added as an QtUiPlus-owned ARTWORK texture and passed
...
StyleUnlearnButton = function(button)
  ...
  if not button.qtpUnlearnIcon and button.CreateTexture then
    ...
    if ok and icon then
      ...
      button.qtpUnlearnIcon = icon
    end
  end

  local options
  if button.qtpUnlearnIcon then
    options = { keep = { [button.qtpUnlearnIcon] = true } }
  end
  ...
  if button.qtpUnlearnIcon then pcall(button.qtpUnlearnIcon.Show, button.qtpUnlearnIcon) end
  ...
  local icon = button.qtpUnlearnIcon
  if icon and not button.qtpUnlearnStates then
    button.qtpUnlearnStates = true
